class BudgetCycleUtils {
  static Map<String, DateTime> getCycleBoundaries(int startDay) {
    ...
    if (now.day >= startDay) {
      ...
      endDate = DateTime(now.year, now.month + 1, startDay).subtract(const Duration(seconds: 1));
    } else {
      // We are in the cycle that started last month
      startDate = DateTime(now.year, now.month - 1, startDay);
      endDate = DateTime(now.year, now.month, startDay).subtract(const Duration(seconds: 1));
    }
